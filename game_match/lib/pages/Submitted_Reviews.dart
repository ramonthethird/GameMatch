...
class _SubmittedReviewsPageState extends State<SubmittedReviewsPage> {
  final FirebaseFirestore firestore = FirebaseFirestore.instance; // Firestore instance to access the database
  final FirebaseAuth auth = FirebaseAuth.instance; // Firebase Authentication instance for user access
  final ApiService apiService = ApiService(); // Instance of ApiService to fetch game details
  ...
  Future<void> _fetchGameDetails(String gameId) async {
    try {
      ...
      print('Error fetching game details: $e'); // Log an error if fetching fails
    }
  }
        ...
        return AlertDialog(
          ...
          content: Container(
            ...
            child: StatefulBuilder(
              builder: (context, setState) {
                void updateRating(double localX) {
                  ...
                  final double totalWidth = starWidth * 5; // Total width of all stars
                  ...
                  setState(() {
                    currentRating = (newRating * 2).round() / 2; // Update the rating to the nearest 0.5
                  });
                }
            ...
            SizedBox(height: 8),
                    ...
                    GestureDetector(
                      onPanUpdate: (details) {
                        updateRating(details.localPosition.dx); // Update rating on drag
                      },
                      onTapDown: (details) {
                        updateRating(details.localPosition.dx); // Update rating on tap
                      },
                      child: Row(
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              SizedBox(width: 4),
                              Text('${review['likes'] ?? 0}', style: TextStyle(fontSize: 12)),
                              SizedBox(width: 16),
                              Icon(Icons.heart_broken, color: Colors.blue, size: 24),
                              SizedBox(width: 4),
                              Text('${review['dislikes'] ?? 0}', style: TextStyle(fontSize: 12)),
                            ],
                          ),
                          Spacer(),
                          IconButton(
                              ...
                              showDialog(
                                ...
                                builder: (context) => AlertDialog(
                                  ...
                                  actions: [
                                    ...
                                    TextButton(
                                      onPressed: () {
                                        _deleteReview(reviewId); // Delete the review
                                        Navigator.of(context).pop();
                                      },
                                      child: Text('Yes'),
                                    ),
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 10),
            Expanded(child: _buildReviewList()), // Build the review list dynamically
          ],
        ),
      ),
    );
